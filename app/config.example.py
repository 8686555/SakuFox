...
PROMPTS = {
    ...
    "report_bundle_user": {
        "en": (
            ...
            "- You may omit irrelevant charts, but do not invent chart ids.\n\n"
              ...
              "- Use a continuous scrollable web-report layout with natural document flow, report-density sections, tables, callouts, and chart areas.\n"
        ...
        "zh": (
            ...
            "- 可以省略无关图表，但不要编造 chart id。\n\n"
              ...
              "- 使用连续可滚动的网页报告布局，保持自然文档流、合理信息密度、章节、表格、重点提示和图表区域。\n"
    ...
    "report_bundle_ppt_user": {
        "en": (
            ...
            "- Include visual hierarchy: gradient backgrounds, card surfaces, KPI callouts, and chart placeholders.\n"
        ...
        "zh": (
            ...
            "- 包含视觉层级：渐变背景、卡片表面、KPI 重点提示和图表占位区域。\n"
            "- html_document 必须包含真实 HTML 标签，不能是 &lt;html&gt; 这类可见转义标签、JSON 文本，或 \\u4e2d、\\n 这类可见转义序列。\n"
            "- 不要包含外部脚本、外部样式表或内联 JavaScript；图表由宿主应用挂载。\n"
            "- 可见文本中不要包含原始 Markdown 语法：不要有 ## 标题、**加粗**、| a | b | 这类管道表格或 ``` 围栏。\n"
            "- 将任何表格内容转换为真正的 <table><thead><tbody> HTML。\n"

...
PROMPTS = {
    "iteration_system": {
        "en": (
            ...
            "Escape code newlines exactly once for JSON; do not double-escape them into visible backslash-n text in the executed code.\n\n"
        ...
        "zh": (
            ...
            "代码换行只按 JSON 要求转义一次，不能双重转义成执行时可见的反斜杠 n 文本。\n"
            "\n"
    ...
    "report_bundle_user": {
        "en": (
            ...
            "- chart_bindings can be an empty array when the HTML is fully self-contained.\n\n"
            ...
            "- Use a continuous scrollable web-report layout with natural document flow, report-density sections, tables, and callouts.\n"
            ...
            "- Decide the layout, sections, typography, emphasis, and visual rhythm yourself; use substantial CSS for spacing, hierarchy, surfaces, and tables.\n"
        ...
        "zh": (
            ...
            "- 当 HTML 已完全自包含时，chart_bindings 可以为空数组。\n\n"
            "HTML 质量要求:\n"
            ...
            "- 使用连续可滚动的网页报告布局，保持自然文档流、合理信息密度、章节、表格和重点提示。\n"
            "- html_document 必须包含真实 HTML 标签，不能是 &lt;html&gt; 这类可见转义标签、JSON 文本，或 \\u4e2d、\\n 这类可见转义序列。\n"
            "- 自行决定布局、章节、字体层级、重点和视觉节奏；使用充分 CSS 处理间距、层级、表面和表格。\n"
    ...
    "report_bundle_ppt_system": {
        "en": (
            ...
            "Design the standalone HTML presentation from the completed iteration results themselves. "
            "Choose the structure, narrative flow, and visual treatment that best fit the evidence; never follow a fixed presentation template. "
        ...
        "zh": (
            ...
            "根据已完成的迭代结果本身设计独立 HTML 演示文稿。请选择最适合证据的结构、叙事流和视觉表达，never follow a fixed presentation template。"
    ...
    "report_bundle_ppt_user": {
        "en": (
            ...
            "- Preferred display style: {display_style_instruction}\n"
            "- chart_bindings can be an empty array when the HTML is fully self-contained.\n\n"
            "PPT slide deck requirements:\n"
            "- Return a complete standalone HTML document with <!doctype html>, <html>, <head>, <style>, and <body>.\n"
            "- The <head> must include <meta charset=\"UTF-8\"> or an equivalent UTF-8 charset declaration.\n"
            "- Design it as a 16:9 slide deck presentation with scroll-snap navigation.\n"
            "- Use CSS scroll-snap-type: y mandatory on the container and scroll-snap-align: start on each slide.\n"
            "- Each slide should have aspect-ratio: 16/9 or min-height: 100vh to fill the viewport.\n"
            "- Each slide contains one major section or topic (title slide, key findings, charts, conclusions, action items).\n"
            "- Use large, bold typography for slide titles (48px+). Use readable body text (24px+).\n"
            "- Include visual hierarchy: gradient backgrounds, card surfaces, KPI callouts, and strong section transitions.\n"
            "- html_document must contain actual HTML tags, not escaped visible tags such as &lt;html&gt;, JSON text, or visible escape sequences.\n"
            "- Do not include external scripts, external stylesheets, or inline JavaScript.\n"
            "- Do NOT include raw Markdown syntax anywhere in visible text: no ## headings, no **bold**, no pipe tables, and no ``` fences.\n"
            "- Convert any tabular content into real <table><thead><tbody> HTML.\n"
            "- Keep content faithful to the iteration evidence; do not add unsupported claims.\n"
        ),
        "zh": (
            ...
            "- 返回完整独立 HTML 文档，包含 <!doctype html>、<html>、<head>、<style> 和 <body>。\n"
            "- <head> 必须包含 <meta charset=\"UTF-8\"> 或等价的 UTF-8 charset 声明。\n"
            "- 设计为 16:9 幻灯片演示文稿，带滚动吸附导航。\n"
            "- 在容器上使用 CSS scroll-snap-type: y mandatory，每个幻灯片上使用 scroll-snap-align: start。\n"
            "- 每张幻灯片应有 aspect-ratio: 16/9 或 min-height: 100vh 以填满视口。\n"
            "- 每张幻灯片包含一个主要章节或主题（标题页、关键发现、图表、结论、行动建议）。\n"
            "- 使用大号粗体标题字体（48px+）。使用可读正文（24px+）。\n"
            "- 包含视觉层级：渐变背景、卡片表面、KPI 重点提示和明确的章节转场。\n"
            "- html_document 必须包含真实 HTML 标签，不能是 &lt;html&gt; 这类可见转义标签、JSON 文本，或 \\u4e2d、\\n 这类可见转义序列。\n"
            "- 不要包含外部脚本、外部样式表或内联 JavaScript。\n"
            "- 可见文本中不要包含原始 Markdown 语法：不要有 ## 标题、**加粗**、| a | b | 这类管道表格或 ``` 围栏。\n"
            "- 将任何表格内容转换为真正的 <table><thead><tbody> HTML。\n"
    ...
    "report_bundle_custom_system": {
        "en": (
            "You are a principal analytics web designer. Produce only JSON for a self-contained analytics HTML page. "
            "Design the standalone HTML page from the completed iteration results themselves, following the user's requested display style. "
            "Choose the structure, narrative flow, and visual treatment that best fit the evidence. "
            "The html_document value must be real browser-renderable HTML, not markdown, escaped visible text, JSON-as-text, or a template conversion."
        ),
        "zh": (
            "你是首席分析网页设计师。请只输出用于自包含分析 HTML 页面的 JSON。"
            "请根据已完成的迭代结果本身，并遵循用户指定的展示方式，设计独立 HTML 页面。"
            "请选择最适合证据的结构、叙事流和视觉表达。"
            "html_document 的值必须是真正可被浏览器渲染的 HTML，不是 markdown、可见转义文本、JSON 文本或模板转换结果。"
        ),
    },
    "report_bundle_custom_user": {
        "en": (
            ...
            "- Return a complete standalone HTML document with <!doctype html>, <html>, <head>, <style>, and <body>.\n"
            "- The <head> must include <meta charset=\"UTF-8\"> or an equivalent UTF-8 charset declaration.\n"
            "- Follow the user-requested display style closely, but keep the page faithful to the analysis evidence.\n"
            "- chart_bindings can be an empty array when the HTML is fully self-contained.\n"
            "- html_document must contain actual HTML tags, not escaped visible tags such as &lt;html&gt;, JSON text, or visible escape sequences.\n"
            "- Do not include external scripts, external stylesheets, or inline JavaScript.\n"
            "- Do NOT include raw Markdown syntax anywhere in visible text.\n"
            "- Convert any tabular content into real <table><thead><tbody> HTML.\n"
            "- Keep content faithful to the iteration evidence; do not add unsupported claims."
        ),
        "zh": (
            ...
            "- 返回完整独立 HTML 文档，包含 <!doctype html>、<html>、<head>、<style> 和 <body>。\n"
            "- <head> 必须包含 <meta charset=\"UTF-8\"> 或等价的 UTF-8 charset 声明。\n"
            "- 尽量贴合用户要求的展示方式，同时保证内容忠于分析证据。\n"
            "- 当 HTML 已完全自包含时，chart_bindings 可以为空数组。\n"
            "- html_document 必须包含真实 HTML 标签，不能是 &lt;html&gt; 这类可见转义标签、JSON 文本，或可见转义序列。\n"
            "- 不要包含外部脚本、外部样式表或内联 JavaScript。\n"
            "- 可见文本中不要包含原始 Markdown 语法。\n"
            "- 将任何表格内容转换为真正的 <table><thead><tbody> HTML。\n"
            "- 内容必须忠于迭代证据，不要增加无支撑声明。"
        ),
    },
    ...
    "html_report_user": {
        "en": (
            ...
            "- Include strong first-screen identity, designed surfaces, clear spacing, responsive layout, and styled tables.\n"
            "- Do not include external scripts, external stylesheets, or inline JavaScript.\n"
        ...
        "zh": (
            ...
            "- 包含强首屏识别、设计化表面、清晰间距、响应式布局和样式化表格。\n"
            "- 不要包含外部脚本、外部样式表或内联 JavaScript。\n"
    ...
    "report_chat_revision_user": {
        "en": (
            ...
            "- The returned HTML will be displayed directly by the tool without post-processing; return the final complete HTML file.\n"
            "- Keep it PPT/presentation-friendly unless the user explicitly requests another form.\n"
            "- Prefer a light, readable theme; do not return a black or near-black blank canvas.\n"
        ...
        "zh": (
            ...
            "- 返回的 HTML 会被工具直接展示，不会再做二次改写；请直接返回最终完整 HTML 文件。\n"
            "- 除非用户明确要求其他形式，否则保持 PPT/演示友好的展示方式。\n"
            "- 优先使用浅色、可读主题；不要返回黑色或近黑色空白画布。\n"
